...
if(isset($show_in_popup) AND $show_in_popup):
    ...
    ?>
    <a href="#" class="pn_inpost_gallery_in_popup" data-popup-title="<?php echo $popup_title ?>" data-popup-width="<?php echo $popup_width ?>" data-popup-max-height="<?php echo $popup_max_height ?>" data-shortcode-key="<?php echo $shortcode_key ?>" data-shortcode-attributes='<?php echo base64_encode(json_encode($attributes)); ?>'><img src="<?php echo InpostGallery::get_image($album_cover, $album_cover_width . 'x' . $album_cover_height) ?>" /></a>
<?php else: ?>
...
//***
    ...
    <?php if(!empty($slides) AND is_array($slides)): ?>
        <div class="yoxview" id="yoxview_<?php echo $unique_id ?>" style="overflow: hidden;">
            <?php foreach($slides as $gallery_item) : ?><a href="<?php echo $gallery_item['imgurl'] ?>" id="yoxview_img_<?php echo $unique_id ?>_<?php echo($counter++); ?>"><img style="<?php echo $styles_img ?>" src="<?php echo InpostGallery::get_image($gallery_item['imgurl'], $thumb_width . 'x' . $thumb_height) ?>" alt="<?php echo $gallery_item['title'] ?>" title="<?php echo $gallery_item['title'] ?>" /></a><?php endforeach; ?>
        </div>
    <?php endif; ?>
    ...
    <script type="text/javascript">
        jQuery(document).ready(function() {
            jQuery("#yoxview_<?php echo $unique_id ?>").yoxview({
                playDelay: <?php echo(isset($js_play_delay) ? $js_play_delay : 3000) ?>,
                autoHideInfo: false
            });
        });
    </script>
